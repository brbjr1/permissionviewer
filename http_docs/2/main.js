...
var finalresult = {FXObjects: [],PermissionSets: [],ApexClassAccess: [],VFPageAccess: [],SystemPermissions: [],AssignedApps: []};
...
/******************Event Handlers *************************************/
	...
	function ShowObjectModal(sobject, ancor)
	{
		...
		if (ofs.OneFieldIsPermissionable == true && ofp)
		{
			...
			j$.each(ofp,function(index, d)
			{
				...
				if (mytype == 'Summary')
				{
					mytype = 'Roll-Up Summary';
				}
				if (d.FieldDescribe.formula != undefined)
				{
					mytype = 'Formula('+mytype+')';
				}
	...
	j$(document).on('shown.bs.tab', 'a[data-toggle="tab"]', function (e) 
	{
    	...
    	if (e.target.hash == "#AssignedApps" && loadphase  <= 2)
    	{
    		j$("#AssignedApps").LoadingOverlay("show",{image : "",fontawesome : fontawesomeloadIcon});
    	}
	});
...
/******************Support Functions *************************************/
	...
	function GetDataforUser(targetUid)
	{
		...
		finalresult = {FXObjects: [],PermissionSets: [],ApexClassAccess: [],VFPageAccess: [],SystemPermissions: [],AssignedApps:[]};
		...
		else
		{
			j$('#userFullName').html('');
			j$('#userRole').html('');
			j$('#userProfileName').html('');
			j$('#userProfileLicenseType').html('');
			j$('#FXPermissionSetTable').html('');
			j$('#FXLicensesTable').html('');
			j$('#fxobjectpermissiontable').html('');
			j$('#fxApexClassestable').html('');
			j$('#fxVisualforcePagestable').html('');
			j$('#FXSystemPermissionsTable').html('');
			
			//j$('#FXAssignedAppsTable').html('');
			///j$("#MainDetail").LoadingOverlay("show",{image : "",fontawesome : fontawesomeloadIcon});
			j$("#MainDetail").LoadingOverlay("show",{image : "",fontawesome : fontawesomeloadIcon});
															...
															QueryRecords(conn, myquery, function(PermissionSetQueryerr, PermissionSetQueryResults)
															{
																try
																{
																	...
																	else
																	{
																		...
																		DescribeAllSObjects(conn, function(DescribeAllSObjectserr, DescribeAllSObjectsResult,Customtabnames,CustomApplications)
																		{
																			try
																			{
																							...
																							QueryRecords(conn, myquery, function(ApexPageQueryerr, ApexPageQueryResults)
																							{
																								try
																								{
																									...
																									else
																									{
																										...
																										if (CustomApplications != undefined)
																										{
																											if (Array.isArray(CustomApplications))
																											{
																												for (var ica2 = 0; ica2 < CustomApplications.length; ica2++)
																												{
																													var caname = CustomApplications[ica2];
																													var calable = caname;
																													var calable2 = caname.split('__');
																													if (calable2.length == 2)
																													{
																														calable = calable2[1];
																													}
																													calable = calable.replace('_', ' ');
																													var si = {
																													GrantedBy: [],
																													HasAccess: false,
																													Label: calable,
																													...
																													finalresult.AssignedApps.push(si);
																												}
																											}
																										}
																										...
																										CustomApplications.push('*');
																										mypackage.types.push(
																										{
																											'members': CustomApplications,
																																	...
																																	try
																																	{
																																		...
																																		else
																																		{
																																			...
																																			if (zipresults.hasItem(profilepackagename) == true)
																																			{
																																				...
																																				if (mydetail.Profile.applicationVisibilities)
																																				{
																																					for (var av1 = 0; av1 < mydetail.Profile.applicationVisibilities.length; av1++)
																																					{
																																						var myAssignedAppvis = mydetail.Profile.applicationVisibilities[av1];
																																						SetAssignedAppResult(myAssignedAppvis, myUser.ProfileId, finalresult, searchprofilename, true);
																																					}
																																				}
																																			}
																																			else
																																			...
																																			if (PermissionSetQueryResults != undefined)
																																			{
																																				for (var ipr44 = 0; ipr44 < PermissionSetQueryResults.length; ipr44++)
																																				{
																																					...
																																					if (zipresults.hasItem(permpackagename) == true)
																																					{
																																						...
																																						if (mydetail.PermissionSet.applicationVisibilities)
																																						{
																																							for (var av1 = 0; av1 < mydetail.PermissionSet.applicationVisibilities.length; av1++)
																																							{
																																								var myAssignedAppvis = mydetail.PermissionSet.applicationVisibilities[av1];
																																								SetAssignedAppResult(myAssignedAppvis, QueryResult.Id, finalresult, QueryResult.Name, false);
																																							}
																																						}
																																					}
																																					else
																																			...
																																			ProcessFinalResult(finalresult, 2, function(result)
																																			{
																																			    ...
																																			    j$("#AssignedApps").LoadingOverlay("hide", true);
																																			});
																																		}
																																	}
																																	catch (err)
	...
	function ProcessFinalResult(result, pass, callback)
	{
		...
		else if (pass == 2)
		{
			...
			var datatableAssignedAppsTable = '';
			if (result.AssignedApps != undefined)
			{
				datatableAssignedAppsTable += '<table id="AssignedAppsTable" class="display" cellspacing="0" width="100%"><thead><tr>';
				datatableAssignedAppsTable += '<th>Label</th>';
				datatableAssignedAppsTable += '<th>Visible</th>';
				datatableAssignedAppsTable += '</tr></thead>';
				datatableAssignedAppsTable += '<tbody>';
				j$.each(result.AssignedApps,function(index, d)
				{
					datatableAssignedAppsTable += '<tr>';
					datatableAssignedAppsTable += '<td>' + d.Label + '</td>';
					datatableAssignedAppsTable += '<td>' + GetSpanPermission(d.HasAccess, true, d.GrantedBy, null, 'AssignedApp') + '</td>'
					datatableAssignedAppsTable += '</tr>';
				});
				datatableAssignedAppsTable += '</tbody>';
				datatableAssignedAppsTable += '</table>';
				j$('#FXAssignedAppsTable').html(datatableAssignedAppsTable);
				
				var AAtable = j$('#AssignedAppsTable').DataTable(
				{
					"scrollCollapse": true,
					"paging": false,
					"order": [0, "asc"],
				...
				AAtable.search('').columns().search('').draw();
			}
		}
		callback(true);
	}

	function SetAssignedAppResult(mypermission, myProfieePermSerId, fresult, proPermName, IsProfile)
	{
		try
		{
			var appname = mypermission.application;
			for (var i2 = 0; i2 < fresult.AssignedApps.length; i2++)
			{
				var myresult = fresult.AssignedApps[i2];
				if (myresult.Name == appname)
				{
					if (mypermission.visible == 'true')
					{
						myresult.HasAccess = true;
						myresult.GrantedBy.push(
						{
							...
							else
							{
								...
								GetAllCustomTabs(conn, function(taberr, tabsresult)
								{
									...
									else
									{
										...
										GetAllCustomApplication(conn,function (caerr, caresults)
										{
											if (caerr)
											{
												callback(caerr, null);
											...
											{
												callback(missing.length > 0 ? err : null, result, tabnames, caresults);
											}
										});
									}
								});
	...
	function GetAllCustomApplication(conn, callback)
	{
		var lstquery = [
		{
		...
		if (hasaccess == true && grantedby && grantedby.length > 0 && ispermissionable)
		{
			...
			j$.each(grantedby,
				function(i2, ri)
				{
					...
					else if (sftype == 'AssignedApp')
					{
						sfurl = "/" + ri.Id.substring(0, 15) + "?s=AssignedApps";
					}
					else
					{
